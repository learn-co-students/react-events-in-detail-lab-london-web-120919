...
describe("<DelayedButton />", () => {
  ...
  it("should call the callback prop after the delay", done => {
    ...
    setTimeout(() => {
      expect(
        spy.calledOnce,
        "The `onDelayedClick` prop was not called after the delay."
      ).to.be.true;
      done();
    }, DELAY + 1);
  });
  ...
  it("should pass the event to the callback prop", done => {
    ...
    setTimeout(() => {
      expect(
        MOCKED_EVENT.persist.calledOnce,
        "The event passed to the callback prop is being pooled"
      ).to.be.true;
      expect(spy.firstCall.args[0]).to.equal(
        MOCKED_EVENT,
        "The event is not being passed to the callback prop."
      );
      done();
    }, DELAY + 1);
  });
